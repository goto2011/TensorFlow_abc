...
卷积神经网络 Inception-v3模型, 识别图片类型.
目的是自己完成图片分类, 不再使用迁移学习.
"""
import glob
import os.path
import random
...
def main(_):
    ...
    my_var.init_layer_variable([
        ["input", [my_var.get_input_width(), my_var.get_input_height(),
                   my_var.get_input_depth()]],
        ["conv", [5, 32]],
        ["max-pool", 2, ["SAME", 2]],
        ["conv", [5, 64]],
        ["max-pool", 2, ["SAME", 2]],
        ["fc", 512, [1, 1, 0.5]],
        ["fc", my_var.get_output_node()]
    ])
    if (DEBUG_FLAG):
        my_var.layer_variable_dump()
    ...
    regularizer = tf.contrib.layers.l2_regularizer(
        my_var.get_regularization_rate())
    ...
    with tf.Session() as sess:
        ...
        for label_index, label_name in enumerate(label_name_list):
            ...
            for index, unused_base_name in enumerate(image_lists[label_name]['testing']):
                ...
                # 给 y_ 赋值
                ground_truth = tf.zeros(
                    my_var.get_output_node(), dtype=tf.float32)
                ground_truth[label_index] = 1.0
                ground_truths.append(ground_truth)
