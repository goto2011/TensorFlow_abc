...
def main(_):
	...
	variable.init_base_variable(input_node, n_classes, 100, 0.01, 0.99, 0.0001, 500, 0.99)
	...
	# 处理学习率、优化方法等。
	train_step = learning_rate.get_train_op(global_step, variable.get_training_steps(), my_loss, variables_averages_op)
	...
	with tf.Session() as sess:
		...
		for i in range(variable.get_training_steps()):
			...
			if i % 100 == 0 or i + 1 == variable.get_training_steps():
				...
				validation_accuracy = sess.run(evaluation_step, feed_dict={x: validation_bottlenecks, y_: validation_ground_truth})
		...
		for label_index, label_name in enumerate(label_name_list):  # 枚举每个类别,如:0 sunflowers
			for index, unused_base_name in enumerate(image_lists[label_name]['testing']):  # 枚举此类别中的测试数据集中的每张图片
				...
				# 给 x 赋值
				bottleneck = get_or_create_bottleneck(sess, image_lists, label_name, index, 'testing')
				bottlenecks.append(bottleneck)
				...
				predict_tensor, test_accuracy = sess.run([final_tensor, evaluation_step], feed_dict={x: bottlenecks, y_: ground_truths})
